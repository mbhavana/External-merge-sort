...
*	Makefile contains shell commands required for execution. This creates two executable          files: 
  +program1 
  +program2
...
* At the prompt type:
$ make
*	The execution of above command gives a message:
  Complier named: program has been compiled
*	Give the following command at the prompt:
./program1 extsort.input output_file1 order memory_limit
./program2 extsort.input output_file2 order memory_limit

*	First argument is name of your input file.
*	Second argument is name of your output file which contains external merge sort records.
*	Third argument is order in which records should be sorted.
+ Give value 0 for ascending order.
+ Give value 1 for descending order.
*  Fourth argument is the memory limit. 
